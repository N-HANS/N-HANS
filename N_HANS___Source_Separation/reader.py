...
class read_seeds:
    ...
    def get_inputs(self):
        if not hasattr(self, 'ph'):
            ...
            self.ph = [tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1, num_features], name='cleanph'),
...
if __name__ == '__main__':
    g = tf.Graph()
    with g.as_default():
        reader = read_seeds(name='test', queuesize=200, min_after_dequeue=100, nthreads=16)
        ...
        queue_stats = tf.stack([x.queue.size() for x in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.QUEUE_RUNNERS)])
